    ...
    !function() {
        ...
        class Copy {
            constructor(el) {
                ...
                this.copyBtn.addEventListener("click", (event => {
                    ...
                    if (navigator.clipboard) navigator.clipboard.writeText(this.hideArea.value).then((() => {
                        this.copySuccess();
                    })).catch((err => {
        ...
        if (null != document.querySelector("[data-submenubtn]")) {
            ...
            if (null != document.querySelector("[data-submenu]")) {
                ...
                if (window.innerWidth <= 767) submenuParams.el.style.height = "0px"; else submenuParams.el.style.height = submenuParams.height;
                window.onresize = () => {
                    if (window.innerWidth <= 767) submenuParams.el.style.height = "0px"; else submenuParams.el.style.height = `${submenuParams.h}px`;
                };
            }
        ...
        if (null != document.querySelector(".inner-rates")) {
        ...
        if (0 != document.querySelectorAll(".inner-item__changes-value").length) for (const changesLabel of document.querySelectorAll(".inner-item__changes-value")) {
            const change = parseInt(changesLabel.innerHTML.replace(",", "").replace("+", ""));
            if (change < 0) changesLabel.classList.add("_graph-minus"); else changesLabel.classList.add("_graph-plus");
        }
        document.addEventListener("DOMContentLoaded", (() => {
            reverseElements();
            cutLongText();
            if (document.querySelector('form[name="chat"]')) new Chat(document.querySelector('form[name="chat"]'));
            if (document.querySelector("[copy]")) new Copy(document.querySelector("[copy]"));
